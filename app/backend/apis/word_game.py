...
async def play_word_game(request: Request):
    try:
        ...
        # Generate a UNIQUE session ID with timestamp to avoid conflicts
        unique_session_id = f"word_game_{int(time.time())}_{str(uuid.uuid4())[:8]}"

        # Create proper state structure with FRESH session
        state: GameState = {
            "route_to": "word_game",
            "number_game_count": 0,  # Reset counters for fresh game
            "word_game_count": 0,
        ...
        # Use the unique session ID for checkpointer
